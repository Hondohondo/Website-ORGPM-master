<!DOCTYPE html>
<html lang="en">
        <head>
            <?php include "property-head.php";?>
                
                <title>ORG Property Management | Team</title>
            </head>
    <body id="property">
        <header class="header" id="teamproperty">


            <?php include "property-nav.php";?>


            <div class="service__container">
                <div class="service-content">
                    <h1 class="text-service">Team</h1>
                </div>
                <hr class="hr-line">
                    <span>ORG Portfolio Management LLC ("ORG") is proud of its dedicated and experienced team of investment professionals. All team members operate with the highest degree of integrity and commitment to the firms's clients.</span>
            </div>


        </header>

        <section class="section-team" id="stickyy">

            <div class="row">
                <div class="col-1-of-4 box">
                    <!-- <img src="img/bio_eschwartz.jpg" alt="Eschwartz"> -->
                    <i class="ion-ios-person icon-big"></i>
                    <h3 class="team-names">John Doe</h3>

                    <div class="team-contacts">
                        <i class="ion-ios-telephone icon-small"></i>
                       (555) 555-5555
                    </div>

                    <div class="team-contacts">
                        <i class="ion-ios-email icon-small"></i>
                        <a href="mailto:webmaster@example.com">email@email.com</a>
                    </div>
                </div>
                <div class="col-1-of-4 box">
                    <!-- <img src="img/bio_jon.jpg" alt="Berns"> -->
                    <i class="ion-ios-person icon-big"></i>
                    <h3 class="team-names">John Doe</h3>

                    <div class="team-contacts">
                        <i class="ion-ios-telephone icon-small"></i>
                        (555) 555-5555
                    </div>

                    <div class="team-contacts">
                        <i class="ion-ios-email icon-small"></i>
                       <a href="mailto:webmaster@example.com">email@email.com</a>
                    </div>
                </div>
                <div class="col-1-of-4 box">
                    <!-- <img src="img/bio_bmcdowell.jpg" alt="Barbara McDowell"> -->
                    <i class="ion-ios-person icon-big"></i>
                    <h3 class="team-names">Jane Doe</h3>

                    <div class="team-contacts">
                        <i class="ion-ios-telephone icon-small"></i>
                        (555) 555-5555
                    </div>

                    <div class="team-contacts">
                        <i class="ion-ios-email icon-small"></i>
                       <a href="mailto:webmaster@example.com">email@email.com</a>
                    </div>
                </div>
                <div class="col-1-of-4 box">
                    <!-- <img src="img/bio_bria.jpg" alt="Bria Macon"> -->
                    <i class="ion-ios-person icon-big"></i>
                    <h3 class="team-names">Jane Doe</h3>

                    <div class="team-contacts">
                        <i class="ion-ios-telephone icon-small"></i>
                        (555) 555-5555
                    </div>

                    <div class="team-contacts">
                        <i class="ion-ios-email icon-small"></i>
                       <a href="mailto:webmaster@example.com">email@email.com</a>
                    </div>
                </div>
                
            </div>
            <div class="row">
                <div class="col-1-of-4 box">
                    <!-- <img src="img/bio_eschwartz.jpg" alt="Eschwartz"> -->
                    <i class="ion-ios-person icon-big"></i>
                    <h3 class="team-names">John Doe</h3>

                    <div class="team-contacts">
                        <i class="ion-ios-telephone icon-small"></i>
                       (555) 555-5555
                    </div>

                    <div class="team-contacts">
                        <i class="ion-ios-email icon-small"></i>
                        <a href="mailto:webmaster@example.com">email@email.com</a>
                    </div>
                </div>
                <div class="col-1-of-4 box">
                    <!-- <img src="img/bio_jon.jpg" alt="Berns"> -->
                    <i class="ion-ios-person icon-big"></i>
                    <h3 class="team-names">John Doe</h3>

                    <div class="team-contacts">
                        <i class="ion-ios-telephone icon-small"></i>
                        (555) 555-5555
                    </div>

                    <div class="team-contacts">
                        <i class="ion-ios-email icon-small"></i>
                       <a href="mailto:webmaster@example.com">email@email.com</a>
                    </div>
                </div>
                <div class="col-1-of-4 box">
                    <!-- <img src="img/bio_bmcdowell.jpg" alt="Barbara McDowell"> -->
                    <i class="ion-ios-person icon-big"></i>
                    <h3 class="team-names">Jane Doe</h3>

                    <div class="team-contacts">
                        <i class="ion-ios-telephone icon-small"></i>
                        (555) 555-5555
                    </div>

                    <div class="team-contacts">
                        <i class="ion-ios-email icon-small"></i>
                       <a href="mailto:webmaster@example.com">email@email.com</a>
                    </div>
                </div>
                <div class="col-1-of-4 box">
                    <!-- <img src="img/bio_bria.jpg" alt="Bria Macon"> -->
                    <i class="ion-ios-person icon-big"></i>
                    <h3 class="team-names">Jane Doe</h3>

                    <div class="team-contacts">
                        <i class="ion-ios-telephone icon-small"></i>
                        (555) 555-5555
                    </div>

                    <div class="team-contacts">
                        <i class="ion-ios-email icon-small"></i>
                       <a href="mailto:webmaster@example.com">email@email.com</a>
                    </div>
                </div>
                
            </div>
            
            
        </section>




        <?php include "property-footer.php";?>

        <?php include "general-footer.php";?>

    </body>
</html>
